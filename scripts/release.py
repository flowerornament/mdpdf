...
def cargo_lock_version() -> str:
    ...
    if match is None:
        fail("could not find mdpdf package entry in Cargo.lock")
    return match.group(1)
...
def flake_version() -> str:
    ...
    if match is None:
        fail('could not find mdpdf buildRustPackage version in flake.nix')
    return match.group(1)


def flake_package_systems() -> list[str]:
    text = read_text(ROOT / "flake.nix")
    match = re.search(r"(?m)^\s*forAllSystems = nixpkgs.lib.genAttrs \[(?P<body>[^]]+)\];$", text)
    if match is None:
...
def insert_changelog_entry(text: str, version: str, today: str) -> str:
    ...
    marker = "All notable changes to `mdpdf` are documented in this file.\n\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Release helper for mdpdf")
